...
#### A-7. メインアクションはメイン画面内で完結させ、クリーチャーの動作でフィードバックする

ごはん・トレーニング・遊びなどの主要アクションは、**別画面（フルスクリーンモーダル）へ遷移させず、メイン画面のクリーチャー表示エリア内で完結**させる。プレイヤーへの反応は、メッセージだけでなく**クリーチャー自身のアニメ動作**で返す（ごはん=eating、遊び=happy、トレ連打中=attack 等）。

- アクションに駆け引き（ミニゲーム）が必要な場合も、モーダルではなくクリーチャー表示エリア内へオーバーレイする。
- 一時的なアニメ演出は transient state（例: `actionAnimation: 'attack'|'eating'|'happy'|null`）で扱い、タイマーで自動解除する。複数アクション分を個別 boolean に増やさず、列挙型に一本化して同時発火の不整合を型で防ぐ。
- 演出中・ミニゲーム中は徘徊アニメ等の自動挙動を止め、その場でアクションを見せる。

> 背景: スプリント `20260620-inline-main-screen-actions`（即時化）と `20260620-training-minigame-creature-action`（連打ミニゲーム＋演出）で確立。手触りの軽さとクリーチャーへの愛着を両立させる方針。
